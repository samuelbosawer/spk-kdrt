   ...
   public function index()
{
    ...
    */
    ...
    // Ambil kasus (pengaduan)
    $nilai = PengaduanMasyarakat::with('nilaiKasus')->where('status','Diterima')->where('id',$id)
        ->when(auth()->check() && auth()->user()->hasRole('masyarakat'), function ($query) {
            $query->where('user_id', auth()->id());
        })
        ->get();
...
// Ambil kasus (pengaduan)
    $pengaduan = PengaduanMasyarakat::with('nilaiKasus')->where('status','Diterima')->where('id',$id)
        ->when(auth()->check() && auth()->user()->hasRole('masyarakat'), function ($query) {
            $query->where('user_id', auth()->id());
        })
        ->first();
    return view('admin.rekomendasi.detail',
        compact(
            'kriteria',
            'alternatif',
            'nilai',
